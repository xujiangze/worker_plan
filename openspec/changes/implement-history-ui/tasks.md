...
**Acceptance Criteria**:
- [ ] 创建 `history.ts` 文件
- [ ] 实现 `getPlanHistory()` 函数,支持分页和筛选参数
- [ ] 实现 `getPlanDetail()` 函数,获取计划详情
- [ ] 定义 TypeScript 类型: `HistoryQueryParams`, `HistoryResponse`
- [ ] 导出所有 API 函数和类型
...
**Acceptance Criteria**:
- [ ] 补充 `PlanHistory` 接口,包含所有必需字段
- [ ] 添加 `ChangeType` 类型定义 (Status, Progress, Info)
- [ ] 添加 `HistoryQueryParams` 接口
- [ ] 添加 `HistoryResponse` 接口
- [ ] 确保类型定义与后端 API 响应一致
...
**Acceptance Criteria**:
- [ ] 创建 Vue 3 Composition API 组件
- [ ] 从路由参数获取 `planId`
- [ ] 定义组件状态: `plan`, `histories`, `loading`, `filters`, `pagination`
- [ ] 实现页面布局: 返回按钮、计划信息卡片、筛选栏、历史记录列表、分页
- [ ] 使用 Element Plus 组件库
...
**Acceptance Criteria**:
- [ ] 使用 `el-card` 或 `el-timeline` 组件展示历史记录
- [ ] 每条记录显示: 变更类型标签、变更时间、字段名称、旧值、新值
- [ ] 历史记录按时间倒序排列
- [ ] 变更类型使用不同颜色: Status(蓝色)、Progress(绿色)、Info(灰色)
- [ ] 处理空历史记录的情况,显示"暂无历史记录"提示
...
**Acceptance Criteria**:
- [ ] 添加变更类型下拉选择器 (`el-select`)
- [ ] 选项包括: 全部、状态、进度、信息
- [ ] 选择变更类型后,自动刷新历史记录列表
- [ ] 更新 URL 查询参数 `change_type`
- [ ] 支持从 URL 查询参数恢复筛选状态
...
**Acceptance Criteria**:
- [ ] 添加时间范围选择器 (`el-date-picker`)
- [ ] 支持选择开始日期和结束日期
- [ ] 选择时间范围后,自动刷新历史记录列表
- [ ] 更新 URL 查询参数 `start_date` 和 `end_date`
- [ ] 支持从 URL 查询参数恢复筛选状态
- [ ] 支持清除时间范围选择
...
**Acceptance Criteria**:
- [ ] 使用 `dayjs` 或 `date-fns` 库格式化日期
- [ ] 变更时间格式: `YYYY-MM-DD HH:mm`
- [ ] 截止日期格式: `YYYY-MM-DD`
- [ ] 使用本地时区
- [ ] 创建可复用的日期格式化函数
...
**Acceptance Criteria**:
- [ ] 在每个计划卡片或表格行中添加"查看历史"按钮
- [ ] 点击按钮跳转到 `/history/:planId`
- [ ] 按钮样式符合设计规范
- [ ] 使用 Element Plus 的 `el-button` 组件
...
- [ ] 移动端(< 768px): 单列布局
- [ ] 筛选栏和分页控件适配移动端
- [ ] 使用 CSS 媒体查询或 Element Plus 的响应式栅格系统
...
**Acceptance Criteria**:
- [ ] 测试所有功能: 列表展示、筛选、分页、加载、错误处理
- [ ] 测试不同场景: 有历史记录、无历史记录、大量历史记录
- [ ] 测试响应式布局在不同设备上的显示效果
- [ ] 优化性能: 减少不必要的重新渲染
- [ ] 优化用户体验: 添加过渡动画、优化加载速度
- [ ] 修复发现的 bug
